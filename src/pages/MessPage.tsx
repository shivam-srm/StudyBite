  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-[var(--gradient-mesh)] bg-no-repeat">
      <div className="container mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        {/* Day Navigation */}
        <div className="flex items-center justify-center gap-2 sm:gap-4 mb-6 animate-fade-in" style={{ animationDelay: "150ms" }}>
          <button
            onClick={goToPrevDay}
            className="p-2 rounded-xl bg-card border border-border hover:bg-muted hover:border-primary/50 transition-all duration-300 hover:scale-110 active:scale-95"
          >
            <ChevronLeft className="w-5 h-5" />
          </button>

          <div className="flex-1 max-w-md overflow-x-auto">
            <div className="flex gap-1.5 sm:gap-2 justify-center p-1 bg-card/50 backdrop-blur-sm rounded-xl border border-border/50">
              {days.map((day) => (
                <button
                  key={day}
                  onClick={() => setSelectedDay(day)}
                  className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap transition-all duration-300 ${
                    selectedDay === day
                ...
                >
                  ...
                  {day === today && selectedDay !== day && (
                    <span className="ml-1 inline-block w-1.5 h-1.5 rounded-full bg-primary animate-pulse-soft" />
                  )}
                </button>
              ))}
            </div>
        ...
        {/* Filter Chips */}
        <div className="flex justify-center flex-wrap gap-2 mb-6 sm:mb-8 animate-fade-in" style={{ animationDelay: "200ms" }}>
          {[
            { key: "all", label: "All", icon: null, activeClass: "bg-primary text-primary-foreground" },
            { key: "veg", label: "Veg", icon: <Leaf className="w-3.5 h-3.5" />, activeClass: "bg-green-500 text-white" },
            { key: "nonveg", label: "Non-Veg", icon: <Drumstick className="w-3.5 h-3.5" />, activeClass: "bg-red-500 text-white" },
            { key: "special", label: "Special", icon: <Sparkles className="w-3.5 h-3.5" />, activeClass: "bg-amber-500 text-white" },
          ].map(({ key, label, icon, activeClass }) => (
            <button
              key={key}
              onClick={() => setFilter(key as FilterType)}
              className={`px-4 py-2 rounded-full text-xs sm:text-sm font-medium flex items-center gap-1.5 transition-all duration-300 tag-animate ${
                filter === key
        ...
        {/* Selected Day Header */}
        <div className="text-center mb-6 animate-fade-in" style={{ animationDelay: "250ms" }}>
          <h2 className="text-xl sm:text-2xl font-semibold text-foreground flex items-center justify-center gap-2">
            {selectedDay === today && (
        ...
        <div className="grid sm:grid-cols-2 gap-4 sm:gap-6 stagger-children">
          {meals.map((meal) => {
            ...
            return (
              ...
              >
                <div
                  className={`p-4 border-b border-border bg-gradient-to-r ${getMealGradient(meal)}`}
                >
                  <div className="flex items-center gap-3">
                    <span className="text-3xl sm:text-4xl group-hover:animate-bounce-soft transition-transform">
                      {info.icon}
                    </span>
                    <div className="flex-1">
                      <h3 className="font-semibold text-base sm:text-lg text-foreground capitalize">
                        {meal}
                      </h3>
                  ...
                  {items.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {items.map((item, idx) => (
                        <span
                          key={idx}
                          className={`px-3 py-1.5 text-xs sm:text-sm rounded-full border transition-all duration-200 tag-animate ${
                            item.isSpecial
                        ...
                        >
                          {!item.isVeg && <span className="mr-1">🍗</span>}
                    ...
                    <div className="text-muted-foreground text-sm text-center py-6 flex flex-col items-center gap-2">
